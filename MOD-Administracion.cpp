...
main(){
	...
	do{								//en este do while valido que la persona que entre al area de administracion
		...
		printf("Ingrese clave de administración: ");
		_flushall();
		gets(intentoclave);
		intentos--;
		system("cls");
...
void usuario(char user[10],FILE *&arc){
	...
	if(user[0]<'a' or user[0]>'z'){
		printf("X La primera letra del usuario ingresado debe ser minúscula\n");
		condiciones--;
	}
	...
	if(may<2){
		printf("X El usuario ingresado debe tener al menos dos letras mayúsculas\n");
		condiciones--;
	}
	if(num>3){
		printf("X El usuario puede tener como máximo tres números\n");
		condiciones--;
	}
	if(otros>0){
...
void registro_usuario_y_contra(FILE *&arc){
	...
	usuario(user,arc);
	printf("Usuario válido, Ingrese una contraseña: ");
	_flushall();
	gets(pass);
	printf("Y un nombre");
	_flushall();
